...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/averywHW4"
...
## Set up Shell context so it's easy to use the shell to debug -- ASK MAULI
...
tweets_hashtags = db.Table('tweets_hashtags',db.Column('tweet_id',db.Integer, db.ForeignKey('Tweets.id')),db.Column('hashtag_id',db.Integer, db.ForeignKey('Hashtags.id')))
...
class User(db.Model):
    ...
    def __repr__(self):
        return "{} (ID: {})".format(self.id,self.twitter_username)
...
# User(twitter_username='Avery')
# x = User(twitter_username='Avery')
# print (db.session.add(x))
# db.session.commit()

def get_or_create_tweet(db_session,text, user_id):
    tweet = db_session.query(Tweet).filter_by(text=text, user_id=user_id).first()
    if tweet:
        return tweet
    else:
...
# print (get_or_create_tweet(db.session, 'text', 1))
...
class TweetForm(FlaskForm):
    text = StringField("What is your tweet?", validators=[Required()])
...
def index():
    ...
    if newform.validate_on_submit():
        user_id=db.session.query(User).filter_by(twitter_username=newform.username.data).first().id
        if db.session.query(Tweet).filter_by(text=newform.text.data, user_id=user_id).first(): # If there's already a song with that title, though...nvm, can't. Gotta add something like "(covered by..)" in the title, or whatever
            flash("You've already tweeted that!") 
        get_or_create_tweet(db.session,newform.text.data, user_id)
    return render_template('index.html', form=newform)
